...
// production. Per-test storage isolation (fresh DO/D1/KV) is the v4 default.
//
// The pool does NOT auto-apply D1 migrations. We read the shared repo-root
// migrations here (Node side) and `provide` them; the test setup file applies them
// to the fresh per-test D1 via applyD1Migrations(env.DB, migrations) in beforeAll.
// Vitest evaluates this config with the project dir (apps/steward) as cwd, so the
// shared repo-root migrations/ resolves at "../../migrations" — the same path the
// wrangler.test.jsonc migrations_dir uses. (Relative string avoids depending on
// @types/node, which the toolchain does not install.)
