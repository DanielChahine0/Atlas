...
Wire the hooks in Phase 0/1 even though Flagger consumes them in Phase 2: a thrown `scheduled()`, a halted Workflow step, a Steward retry-exhaustion, and a DLQ landing each emit a Flagger Wire event `{agent, type, entity:"flagger", op:"increment", payload:{severity, trust, kind}, idempotencyKey}`. Until the Flagger Worker exists, those events are still recorded by Steward into `run_log`/`audit_log`, so the history is intact. **[SUPERSEDED — reconciled in 00-02]** This `entity:"flagger"`/`op:"increment"` shape is a pre-Flagger stub; the canonical Flagger event (SPEC-CANON §8 / docs/08-flagger.md §4/§6) is `{agent, type:"flag", entity:"flag", op:"upsert", payload:<full flag record {id,ts,source_agent,severity,trust,title,detail,suggested_action,status}>, idempotencyKey:id}` — a flag is a STABLE ROW keyed by `id`, mutated in place ⇒ `upsert`, not `increment`. This is what `packages/shared` `flag()` emits.
