...
- **D2-11: Friday concurrency is build-plan-locked.** One cron `"0 21 * * 5"` (EDT) runs Scout-weekly **and** weekly-Herald via `Promise.all` (disjoint sources, both fan into Steward); `"30 21 * * 5"` kicks the 16:30 weekly-review build. These are **standalone crons in Atlas's `scheduled()` switch — NOT the morning Workflow.** (UTC/DST hand-edit per D1; `/cron-utc` to translate.)
