...
- **Hard prerequisite:** Cloudflare Workers **Paid** plan — Queues (the Wire), Workflows, and KV-backed Durable Objects require it. Treat as a Phase-0 line item.
...
- **Tech stack / hosting**: Cloudflare Workers Paid plan; the Cloudflare primitive map (Workers / DOs / Queues+DLQ / Workflows / Cron / D1 / KV / R2) and the MCP connectivity contract (Google OAuth2 least-privilege, GitHub App, local Obsidian bridge) are fixed by SPEC §7 / docs/06.
